...
fn draw_left_panel(
    ...
    player_q: &Query<(Entity, &StationedAt, Option<&AboardShip>), With<Player>>,
    positions: &Query<&Position>,
    system_planets: &[(Entity, String, String, bool, Option<Habitability>)],
    selected_planet: &mut SelectedPlanet,
...
fn draw_system_map(
    ...
    for (i, (planet_entity, name, _planet_type, is_colonized, hab)) in system_planets.iter().enumerate() {
        ...
        let planet_radius = match hab {
            Some(Habitability::GasGiant) => 14.0,
            _ => 9.0,
        };
        ...
        if let Some(hab) = hab {
            let hab_str = match hab {
                Habitability::Ideal => "Ideal",
                Habitability::Adequate => "Adequate",
                Habitability::Marginal => "Marginal",
                Habitability::GasGiant => "Gas Giant",
                Habitability::Barren => "Barren",
            };
            painter.text(
