...
#[test]
fn test_colony_ship_establishes_colony() {
    let mut app = test_app();
    ...
    // Spawn colony ship already docked at System-B
    let ship_entity = app.world_mut().spawn((
        Ship {
            name: "Colony-1".to_string(),
            ship_type: ShipType::ColonyShip,
    ...
    // System-B should now be colonized
    let star = app.world().get::<StarSystem>(sys_b).unwrap();
    assert!(star.colonized, "System-B should be colonized");

    // Colony entity should exist
    let mut colony_query = app.world_mut().query::<&Colony>();
    let colonies: Vec<_> = colony_query.iter(app.world()).collect();
    assert_eq!(colonies.len(), 1, "Exactly one colony should exist");
    assert_eq!(colonies[0].system, sys_b, "Colony should reference System-B");

    // Colony ship should be despawned (deferred commands apply after update)
    // Need another update to flush commands
    app.update();
    assert!(
        app.world().get_entity(ship_entity).is_err(),
        "Colony ship should be despawned"
    );
}

#[test]
fn test_colony_ship_rejects_gas_giant() {
    let mut app = test_app();

    let sys_gas = spawn_test_system(
        app.world_mut(),
        "Gas-Giant-1",
        [5.0, 0.0, 0.0],
    ...
    let ship_entity = app.world_mut().spawn((
        ...
        ShipState::Docked { system: sys_gas },
        Position::from([5.0, 0.0, 0.0]),
    )).id();

    app.update();

    // System should NOT be colonized
    let star = app.world().get::<StarSystem>(sys_gas).unwrap();
    assert!(!star.colonized, "Gas giant should not be colonized");

    // Ship should still exist
    assert!(
        app.world().get_entity(ship_entity).is_ok(),
        "Colony ship should still exist"
    );

    // No colony entities should exist
    let mut colony_query = app.world_mut().query::<&Colony>();
    let colonies: Vec<_> = colony_query.iter(app.world()).collect();
    assert!(colonies.is_empty(), "No colony should be created on gas giant");
}
